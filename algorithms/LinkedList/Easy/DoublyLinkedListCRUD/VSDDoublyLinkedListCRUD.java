...
/*Java program for CRUD operations on double linked list*/
	...
	static Scanner sc=new Scanner(System.in);
		...
		public static void VSDinsertAtEnd() {
			...
			if(head==null)head=temp;//Inserting first element as head
				...
				x.next=temp;//Inserting node at end of Double Linked list
				temp.previous=x;
				}
				VSDprintList();
				}
			...
			if(head==null)head=temp;
				...
				System.out.println("Please Enter element from Double Linked list to insert after:");
				int num=sc.nextInt();
				...
				if(flag==0)
					...
					if(x.next.previous!=null)x.next.previous=temp;
					temp.next=x.next;
					x.next=temp;
					temp.previous=x;
					}
			...
			public static void VSDdelete(){
				if(head== null)//Checking if the list is null
					...
					while(x!=null) {
						if(x.data==num) {
							if(x.next==null)x.previous.next=x.next;
							else {
			...
			//Function to destroy Double Linked List
			...
			//Function to print the Double Linked List		
				public static void VSDprintList() {
							...
							if(head== null)
								...
								System.out.println("Now the Double Linked List is");
							VSDNode x=head;
							while(x!=null) {
			...
			public static void VSDuserInput() {
					System.out.println("Please Enter Double Linked List operation from:");
			...
			public static void main(String args[]) {
				...
				VSDuserInput();//Taking user input to perform CRUD operations on Double Linked List
